...
prompt_context() {
  if [[ "$USER" != "$DEFAULT_USER" || -n "$SSH_CLIENT" ]]; then
    prompt_segment 22 40 "%(!.%{%F{yellow}%}.)${USER:0:3}@%m"
  fi
}
...
prompt_virtualenv() {
  ...
  if [[ -n $virtualenv_path && -n $VIRTUAL_ENV_DISABLE_PROMPT ]]; then
    prompt_segment 17 21 "(`basename $virtualenv_path`)"
  fi
}
...
prompt_battery() {
  ...
  if [[ "Discharging" == $(< /sys/class/power_supply/BAT0/status) ]]; then
    ...
    if ((30 < bat_pct)); then
      prompt_segment 28 154 "$bat_pct%%"
    elif ((15 < bat_pct)); then
      prompt_segment 208 190 "$bat_pct%%"
    else
      prompt_segment 52 196 "$bat_pct%%"
    fi
  fi
}
